...
# - Tune models (GB, RF, ET, HGB) with RandomizedSearchCV
...
# ----------------------------- CONFIG -----------------------------
BASE_FEATURES = [
    "DOM+","40 Time","BMI","Height","Weight","MTF/A","YPC","YPR","ELU","YCO/A",
...
def save_pareto_chart(summary: pd.DataFrame, position: str, out_dir: Path) -> Path:
    ...
    # label each point with n_subsets
    ...
    # connect points in order of n_subsets
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Position-aware wide search with runtime profiling.")
